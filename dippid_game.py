...
# fruit images from https://pixabay.com/de/illustrations/fr%c3%bcchte-banane-orange-apple-papaya-3658159/
# basket image from https://pixabay.com/de/vectors/korb-picknick-brown-griffe-objekt-310061/
import random
...
class DippidGame(QtWidgets.QWidget):
    def __init__(self, port="5700"):
        ...
        self.dippid = None
        self._acc_val_x = 0
        self.sensor_update_rate_per_s = 20 # rate at which accelerometer data gets refreshed
    ...
    def connect_dippid(self):
        ...
        self.dippid.register_callback('button_1', self.decrement_current_player_type)
        self.dippid.register_callback('button_3', self.increment_current_player_type)
    ...
    def increment_current_player_type(self, data):
        if int(data) != 0:
            if self._current_player_type_id+1 < len(self._player_types_list):
                ...
                self._current_player_type_id = 0
            self._add_image_to_label(self._player_icon, self._player_types_list[self._current_player_type_id]["image"])
    ...
    def start_or_pause_game(self):
        if not self._is_game_running:
            self.set_graphics_update_rate_per_s(self.graphics_update_rate_per_s)
    ...
    def _create_new_fruits(self, amount):
        for i in range(amount):
            ...
            fruit_x = random.randint(0, self._current_frame_geometry.width()-self._fruit_width)
            fruit_y = random.randint(-15, 0)*2
            fruit_id = random.randint(0, len(self._fruit_types_list)-1)
            new_fruit.setGeometry(fruit_x, fruit_y-self._fruit_height, self._fruit_width, self._fruit_height)
            self._add_image_to_label(new_fruit, self._fruit_types_list[fruit_id]["image"])
            new_fruit_speed = random.randint(self._min_fruit_speed, self._max_fruit_speed)
    ...
    def move_fruits(self):
        for fruit, speed, fruit_id in self._fruit_list:
            ...
            if fruit.y() > self._current_frame_geometry.height():
                ...
                fruit.setGeometry(fruit_x, fruit_y-self._fruit_height, self._fruit_width, self._fruit_height)
